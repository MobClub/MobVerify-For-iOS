...
typedef NS_ENUM(NSUInteger, MobVerifyUIProcessHUDStyle)
{
    MobVerifyUIProcessHUDStyleDefault = 0,  //默认
    MobVerifyUIProcessHUDStyleMsg = 1 //文本显示
};

@interface MobVerifyUIProcessHUD()
...
@implementation MobVerifyUIProcessHUD

+ (instancetype)shareInstance
{
    static MobVerifyUIProcessHUD *shareInstance = nil ;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shareInstance = [[MobVerifyUIProcessHUD alloc] init];
    });
...
- (void)setup
{
    ...
    self.successImage =
    ({
        NSString *successImagePath = [MobVerifyUIBundle pathForResource:@"success@3x" ofType:@"png"];
    ...
    self.failedImage =
    ({
        NSString *errorImagePath = [MobVerifyUIBundle pathForResource:@"error@3x" ofType:@"png"];
...
{
    MobVerifyUIProcessHUD *hud = [MobVerifyUIProcessHUD shareInstance];
...
+ (void) showMsgHUDWithInfo:(NSString *)info
{
    ...
    [self resetViewFrame:hud style:MobVerifyUIProcessHUDStyleMsg];
...
+ (void) dismiss
{
    [[MobVerifyUIProcessHUD shareInstance].window resignKeyWindow];
    [MobVerifyUIProcessHUD shareInstance].window = nil;
}
...
+ (void)resetViewFrame:(MobVerifyUIProcessHUD *)hud style:(MobVerifyUIProcessHUDStyle)style
{
    CGSize size = [UIScreen mainScreen].bounds.size ;
    if(style == MobVerifyUIProcessHUDStyleDefault)
    {
    ...
    else if(style == MobVerifyUIProcessHUDStyleMsg)
    {
